...
output_folder = "C:/Users/sohan/Desktop/sop 3-1/my-model/outputs3"
...
for contour in contours:
    # Find the convex hull to detect the base of the piece
    hull = cv2.convexHull(contour)
    lowest_point = tuple(hull[hull[:, :, 1].argmax()][0])  # Lowest point (base)

    # Check which square the base belongs to
    base_x, base_y = lowest_point
    for row in range(rows):
        for col in range(cols):
            cell_x1 = vertical_lines[col]
            cell_x2 = vertical_lines[col + 1]
